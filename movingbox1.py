...
class MovingBox:
    ...
    def __init__(self, box_name):
        #define the attribute of name for the boxes (objects) 
        self.__box_name__ = box_name
        #define a dictionary to store name of items (key) and the number of items (values)
        self.__box_items = {}
    
    def get_box_name(self):
        #get the name of an object for printing
        return self.__box_name__
    
    def add_item(self, item_name, item_count):
        #add item to each box objects
        #first create a new key and value if it dose not make it before
    ...
    def remove_item(self, item_name, item_count):
        #firs: check if a name is in item dictionary
        if item_name in self.__box_items:
        #check if the number of items is larger than the user command/if not return to error message in relevant function   
    ...
    def transfer_item(self, target_name, item_name, item_count):
        #remove the amount of an item from the source/ then add the same into target name
        #for transfer an item, use add and remove method
        if not self.remove_item(item_name, item_count):
            return
        target_name.add_item(item_name, item_count)
        return True
        #return true to hande transfer function error message
    
    def list_content(self):
        #print the key and values of item dictionary after get the related box_name
        print(f'Box "{self.get_box_name()}" contains {sum(self.__box_items.values())} items.')
        for item in sorted(self.__box_items):
            if self.__box_items[item] == 0:
                pass
            else:
                print(f'{self.__box_items.get(item):3} {item}')

    def list_box_content(self, all_boxes, box_name):
        #print the item dictionaries of all objects (boxes)
        print(f'Box "{self.get_box_name()}" contains {sum(self.__box_items.values())} items.')
        for item in sorted(all_boxes[box_name]):
            if all_boxes[box_name] == 0:
                pass
            print(f'{self.get_box_items[item]:3} {item}')
        
    def search_item(self, item_name):
        #search for an item in all boxes/ if an item_name (key) is not in the dic. return ZERO to avoid error message 
        #in search function
        if item_name in self.__box_items:
