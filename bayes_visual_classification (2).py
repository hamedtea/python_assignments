...
def k_NN_classifier(X_train, Y_train, X_test):
...
predictions = k_NN_classifier(X_train, Y_train, X_test)
...
def cifar10_color(X_train,X_test):
  ...
  Xp = np.zeros((len(X_train),3))
  x_test = np.zeros((len(X_test),3))
  for i in range(0,len(X_test)):
    for j in range(0,3):
      Xp[i,j] = resize(X_train[i,...,j], (1,1))
  for i in range(0,len(X_test)):
    for j in range(0,3):
      x_test[i,j] = resize(X_test[i,...,j], (1,1))
  return Xp, x_test

Xp, X_test = cifar10_color(X_train,X_test)
print(Xp.shape)
...
Xf = Xp
mu, sigma, p = cifar_10_bayes_learn(Xf, Y_test)
#print(mu.shape, sigma.shape)
print(sigma.shape, Xf.shape, mu.shape)
...
def cifar_10_classifier_bayes(X_test,mu,sigma,p):
  ...
  for i in range(0,len(X_test)):
    for j in range(0,3):
      probability[j,] = multivariate_normal.pdf(X_test[i,], mu[j], sigma[j,...], allow_singular=True)
    ind = np.argmax(probability)
    classifier[i,] = ind
  return classifier

clasifier = cifar_10_classifier_bayes(X_test,mu,sigma,p)
...
def neural_netwrok_simple(X_train, Y_train, X_test, Y_test):
  ...
  simple_model.fit(X_train_n, encoded_Y_train, epochs=100, callbacks=[callback], verbose=1, shuffle = True)
  pri=simple_model.predict(X_test_n)
  simple_NN_predictions = np.argmax(pri, axis = 1)
  return simple_NN_predictions

simple_NN_predictions = neural_netwrok_simple(X_train, Y_train, X_test, Y_test)
print(simple_NN_predictions[1:100,])
accc = class_acc(simple_NN_predictions,Y_test)
print("the accuracy of Neural Network model is", accc)
...
print(NN_predictions[5000:5800,])
acc = class_acc(NN_predictions,Y_test)
print("the accuracy of Neural Network model is", acc)
...
model.add(Dense(5, input_dim = 3072, activation='sigmoid'))
  ...
  num_of_epochs = 20
  tr_hist = model.fit(X_norm, encoded_Y_labels, verbose=1)